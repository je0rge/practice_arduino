...
void RGB::Init(int r, int g, int b){
  _redPin = r;
  _greenPin = g;
  _bluePin = b;
  pinMode(r, OUTPUT);
...
void DigitalSensor::Init(int p, int m, bool i){
  _state = LOW;
  _pin = p;
  _mode = m;
  _inverted = i;
...
void AnalogSensor::Init(int p, int m, int t, bool i, float cm, float cb){
  _inverted = i;
  _state = 0;
  _pin = p;
  _mode = m;
...
void Buzzer::Init(int p, int ps, int wnp){
    _pin = p;
    _pauseScale = ps;
...
void Button::Init(int p, int dd){
  _pin = p;
  _debounceDelay = dd;
  pinMode(_pin, INPUT_PULLUP);
  _buttonState = digitalRead(_pin); //added
  _lastButtonState = _buttonState;  //added
  _lastDebounceTime = millis();     //added
}
...
void SevenSegment::Init(int p[], int c){
  _isCommonCathode = c;
  for(int i=0; i<8; i++){
    _pin[i] = p[i];
...
void Sweeper::Init(int s, int e, int st, byte x) {
...
void Metronome::Init(unsigned long t){
  _interval= t;
...
void Sequencer::Init(LedsManager* LM){
  _state = 0;
  _type = 0;
  ledsManager = LM;
}
