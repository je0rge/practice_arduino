...
void setup(){ 
    pinMode(10, OUTPUT); // configure pin of buzzer
...
void loop(){  
    ...
    int val = analogRead(0); //Connect LM35 to analog pin and read value from it
    data = (double) val * (5/10.24); //Convert the voltage value to temperature value 
    if(data > 29){ //If temperature is higher than 27, the buzzer starts to make sound. 
     ...
     if(millis() - tepTimer > 500){ // Every 500 ms, serial port outputs temperature value. 
       tepTimer = millis();
